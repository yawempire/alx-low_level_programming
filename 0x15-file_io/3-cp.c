#include "main.h"
#define BUFSIZE 1024


/**
  * error_exit - printing an error message and exit the program
  * @message: error message to be printed
  * @exit_code: the exit code
  */
void error_exit(const char *message, int exit_code)
{
	dprintf(STDERR_FILENO, "%s\n", message);
	exit(exit_code);
}

/**
  * copy_file -  the content of one file to another
  * @from: descriptor file source
  * @to: destination file descriptor
  */
void copy_file(int from, int to)
{
	ssize_t n;
	char buf[BUFSIZE];

	while ((n == read(from, buf, BUFSIZE)) > 0)
	{
		if (write(to, buf, n) != n)
		{
			error_exit("Error: Can't write to the destination file", 99);
		}
	}

	if (n == -1)
	{
		error_exit("Error: Can't read from the source file", 98);
	}
}

/**
  * main - The content of a file copies
  * @argc: number of args
  * @argv: array of args
  *
  * Return: (0) Success, or error on failure
  */
int main(int argc, char *argv[])
{
	int fd_from, fd_to;
	ssize_t n;
	char buf[BUFSIZE];

	if (argc != 3)
	{
		error_exit("Usage: cp file_from file_to", 97);
	}

	fd_from = open(argv[1], O_RDONLY);

	if (fd_from == -1)
	{
		error_exit("Error: Can't read from the source file", 98);
	}

	fd_to = open(argv[2], O_WRONLY | O_TRUNC | O_CREAT, 0064);

	if (fd_to == -1)
	{
		error_exit("Error: Can't write to the destination file", 99);
	}

	copy_file(fd_from, fd_to);

	if (close(fd_from) == -1 || close(fd_to) == -1)
	{
		error_exit("Error: Can't close file descriptor", 100);
	}

	return (0);
}
